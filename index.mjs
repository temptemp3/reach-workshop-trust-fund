...
const runDemo = async (delayReceiver, delayFunder) => {
  ...
  const accBystander = await stdlib.newTestAccount(startingBalance);

  const ctcFunder = accFunder.deploy(backend);
  const ctcReceiver = accReceiver.attach(backend, ctcFunder.getInfo());
  const ctcBystander = accBystander.attach(backend, ctcFunder.getInfo());

  await Promise.all([
    backend.Funder(ctcFunder, {
      ...common('Funder', fDelay),
      getParams: () => ({
        receiverAddr: accReceiver.networkAccount,
  ...
  for (const [who, acc] of [
    ['Funder', accFunder],
    ['Receiver', accReceiver],
    ['Bystander', accBystander],
  ]) {
